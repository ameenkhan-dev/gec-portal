...
const getMyEvents = async (req, res) => {
  try {
    const conn = await pool.getConnection();
    const [events] = await conn.execute('SELECT * FROM events WHERE club_id IN (SELECT club_id FROM club_members WHERE user_id = ?)', [req.user.user_id]);
    conn.release();
    res.json(events);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
module.exports = { getAllEvents, getEventById, createEvent, getMyEvents };
